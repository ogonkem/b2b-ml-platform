...
5. **synthesize** — the only LLM call, provider-selectable via `LLM_PROVIDER` (`openai` → `gpt-4o-mini`, chosen for cost since the prompt is heavily grounded already; `groq`; or a local `ollama`) — explicitly instructed to cite the statistical factors *and* the specific retrieved clause/section, quote threshold values verbatim from what was actually triggered, and never invent a number or policy detail not present in the prompt's own grounding data. Groq and Ollama both expose an OpenAI-compatible chat-completions endpoint, so `agent/clients.py` reaches all three through the same `openai` SDK, just pointed at a different `base_url`/key/model — no extra dependency. This only changes which model *writes the narrative*; embeddings (ingestion and retrieval) stay OpenAI-only regardless of `LLM_PROVIDER`, so a real `OPENAI_API_KEY` is still required for the retrieve step to succeed at all.
